...
## 8) มุมมองข้อมูลจาก View: dbo.vw_mst_org_hierarchy_detail

`dbo.vw_mst_org_hierarchy_detail` เป็นมุมมองข้อมูลโครงสร้างสายบังคับบัญชาแบบ "ดูจบในที่เดียว" โดยรวมจากหลายตารางเรียบร้อยแล้ว

ข้อมูลที่แสดงหลักๆ มี 6 กลุ่ม

### 1. ข้อมูลแถวต้นทางของ hierarchy

- hierarchy_id
- channel_id
- effective_month
- salesman_code, direct_sup_code, dept_mgr_code, div_mgr_code, ad_code
- is_active, created_at, updated_at

### 2. ข้อมูลช่องทางขายจาก mst_channel
...
### 3. ข้อมูลงวดเวลา (period context) จาก mst_period
...
### 4. ข้อมูลพนักงานระดับ Salesman (map จาก salesman_code)

- salesman_employee_id
- salesman_name_th
- salesman_name_en

### 5. ข้อมูลพนักงานสายบังคับบัญชาแต่ละระดับ

- direct_sup_employee_id, direct_sup_name_th, direct_sup_name_en
- dept_mgr_employee_id, dept_mgr_name_th, dept_mgr_name_en
- div_mgr_employee_id, div_mgr_name_th, div_mgr_name_en
- ad_employee_id, ad_name_th, ad_name_en

### 6. ความหมายเชิงใช้งาน

- ใช้ดูโครงสร้างรายเดือนต่อช่องทางขาย
- ใช้ตรวจว่า code ต่างๆ map เข้าข้อมูลพนักงานได้หรือไม่
- ใช้ทำรีพอร์ต/สไลด์โดยไม่ต้องเขียน join หลายตารางเอง
