...
## สถานะล่าสุด (Last updated: 2026-07-08)

โปรเจกต์อยู่ในสถานะ **"production-ready foundation + formula-driven SP"** —
`mst_formula_expression` ใช้งานได้ผ่าน database โดยตรงครบสมบูรณ์แล้ว:
SP อ่าน `formula_expr` ผ่าน `dbo.fn_apply_mt_formula()` ทุกครั้งที่รัน
ยืนยันแล้ว: UPDATE catalog → EXEC SP → ForHR เปลี่ยน 4,071 → 4,152 (+81) ไม่ต้อง API
...
### เซสชันที่ 2 (2026-07-08) — Formula-Driven SP

- **สร้าง `dbo.fn_apply_mt_formula()`**: function ที่อ่าน `formula_expr` จาก
  `mst_formula_expression` runtime, parse token variables (`[base_rate]`, `[weight_pct]`,
  `[goal_mult]`), extract extra multiplier เช่น `* 1.02`, apply ROUND() ตาม pattern
- **ALTER `usp_run_mt_incentive_calculation`**: INSERT 1 และ INSERT 2 (main products + sub-variant)
  ใช้ `dbo.fn_apply_mt_formula()` แทน hardcode T-SQL เดิม
- **DDL script**: [database/ddl/56_formula_driven_mt_sp.sql](../database/ddl/56_formula_driven_mt_sp.sql)
  พร้อม unit tests (T1_baseline PASS, T2_plus2pct PASS)
- **ทดสอบ end-to-end จริง**: baseline run=2065 (4,071) → UPDATE `* 1.02` → run=2066 → ForHR=4,152
  (**+81 บาท, ~2%**) ✅ ผ่าน database โดยตรงทั้งหมด ไม่ต้องใช้ API
- **ทบทวนและแก้เอกสาร** [docs/06-presenatation/AJT_MT_Demo2_FormulaChange_StepByStep.md](../docs/06-presenatation/AJT_MT_Demo2_FormulaChange_StepByStep.md):
  ลบ API reference ทั้งหมด, อัปเดตผลลัพธ์จริง (4,152 ไม่ใช่ 4,153),
  อธิบาย rounding behavior (raw precision vs intermediate ROUND)
- **พบบัคสำคัญรอบ 2 ของการทบทวนเอกสาร**: `usp_run_mt_incentive_calculation` มีคำสั่ง
  `DELETE FROM trn_calc_run WHERE channel_id=@ChannelId AND period_id=@PeriodId` ก่อน INSERT ชุดใหม่ทุกครั้ง —
  หมายความว่า **ไม่สามารถเก็บ 2 calc_run_id ของ period เดียวกันไว้เทียบ before/after พร้อมกันได้**
  (baseline run จะถูกลบทันทีที่ EXEC ซ้ำ) — แก้โดยเพิ่ม pattern **snapshot ผล baseline ลงตารางถาวร**
  (`dbo.tmp_mt_demo2_baseline`) ก่อนแก้สูตรเสมอ — ทดสอบ end-to-end อีกครั้งจนครบถ้วน
  (calc_run_id 2069→2070, 4,071→4,152, +81) แล้วอัปเดตเอกสารทุก section ให้ใช้ pattern นี้แทน
