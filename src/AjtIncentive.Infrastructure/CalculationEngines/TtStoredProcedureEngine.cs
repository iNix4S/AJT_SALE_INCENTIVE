...
/// <summary>
/// Engine 1: เรียก dbo.usp_run_tt_incentive_calculation ตรงๆ (engine ดั้งเดิม)
/// </summary>
public sealed class TtStoredProcedureEngine : ITtCalculationEngine
{
    private readonly string _connectionString;

    public TtStoredProcedureEngine(string connectionString)
    {
    ...
    public async Task<int> RunAsync(string periodCode, string wsType, string? approvedBy = null)
    {
        ...
        if (!calcRunId.HasValue)
            throw new InvalidOperationException("TT calculation (StoredProcedure engine) finished but calc_run_id was not found.");
