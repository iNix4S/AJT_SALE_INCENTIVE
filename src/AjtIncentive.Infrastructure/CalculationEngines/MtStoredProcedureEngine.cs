...
/// <summary>
/// Engine 1: เรียก dbo.usp_run_mt_incentive_calculation ตรงๆ (engine ดั้งเดิม)
/// </summary>
public sealed class MtStoredProcedureEngine : IMtCalculationEngine
{
    private readonly string _connectionString;

    public MtStoredProcedureEngine(string connectionString)
    {
    ...
    public async Task<int> RunAsync(int periodId, string? approvedBy = null)
    {
        ...
        var exists = await conn.ExecuteScalarAsync<int>(
            "SELECT CASE WHEN OBJECT_ID(N'dbo.usp_run_mt_incentive_calculation', N'P') IS NULL THEN 0 ELSE 1 END;");

        if (exists == 0)
            throw new InvalidOperationException("MT calculation is not available because SP usp_run_mt_incentive_calculation is not deployed.");
        ...
        if (!calcRunId.HasValue)
            throw new InvalidOperationException("MT calculation (StoredProcedure engine) finished but calc_run_id was not found.");
