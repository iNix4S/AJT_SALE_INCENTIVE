...
/// <summary>
/// Engine 1: เรียก dbo.usp_run_laos_incentive_calculation ตรงๆ (engine ดั้งเดิม)
/// </summary>
public sealed class LaosStoredProcedureEngine : ILaosCalculationEngine
{
    private readonly string _connectionString;

    public LaosStoredProcedureEngine(string connectionString)
    {
    ...
    public async Task<int> RunAsync(string periodCode, string? approvedBy = null)
    {
        ...
        if (!calcRunId.HasValue)
            throw new InvalidOperationException("LAOS calculation (StoredProcedure engine) finished but calc_run_id was not found.");
