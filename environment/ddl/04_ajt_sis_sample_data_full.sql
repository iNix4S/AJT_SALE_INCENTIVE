...
DECLARE @jf_div  INT = (SELECT job_function_id FROM dbo.mst_job_function WHERE job_function_code = 'DIV_MANAGER');
DECLARE @jf_ad   INT = (SELECT job_function_id FROM dbo.mst_job_function WHERE job_function_code = 'ASSOC_DIRECTOR');
DECLARE @pl_dept INT = (SELECT position_level_id FROM dbo.mst_position_level WHERE position_code = 'DEPT_MGR');
DECLARE @pl_div  INT = (SELECT position_level_id FROM dbo.mst_position_level WHERE position_code = 'DIV_MGR');
DECLARE @pl_ad   INT = (SELECT position_level_id FROM dbo.mst_position_level WHERE position_code = 'AD');

INSERT INTO dbo.mst_employee
    (employee_code, employee_name_th, employee_name_en, channel_id, job_function_id, position_level_id,
     cost_center, company_code, effective_from, effective_to, is_active)
SELECT v.employee_code, v.employee_name_th, v.employee_name_en, @ch_tt_emp, v.job_function_id, v.position_level_id,
       'CC-TT-01', 'AJT', '2026-04-01', NULL, 1
FROM (VALUES
    ('TSM001', N'นาย ฉ. คุมทีม', 'Mr. F Teamlead', @jf_section, @pl_sect),
    ('TDM001', N'นาง ช. ดูแผนก', 'Mrs. G Deptcare', @jf_dept, @pl_dept),
    ('TDV001', N'นาย ฌ. รวมพลัง', 'Mr. H Divsync', @jf_div, @pl_div),
    ('TAD001', N'นาง ญ. บริหาร', 'Mrs. I Director', @jf_ad, @pl_ad)
) v(employee_code, employee_name_th, employee_name_en, job_function_id, position_level_id)
WHERE NOT EXISTS (
    SELECT 1 FROM dbo.mst_employee e WHERE e.employee_code = v.employee_code
);
...
IF @ch_tt_emp IS NOT NULL
INSERT INTO dbo.mst_org_hierarchy
    (channel_id, effective_month, salesman_code, direct_sup_code, dept_mgr_code, div_mgr_code, ad_code, is_active)
SELECT @ch_tt_emp, '2026-04-01', v.salesman_code, v.direct_sup_code, v.dept_mgr_code, v.div_mgr_code, v.ad_code, 1
FROM (VALUES
    ('TT001', 'TSM001', 'TDM001', 'TDV001', 'TAD001'),
    ('TT002', 'TSM001', 'TDM001', 'TDV001', 'TAD001')
) v(salesman_code, direct_sup_code, dept_mgr_code, div_mgr_code, ad_code)
WHERE NOT EXISTS (
    SELECT 1
    FROM dbo.mst_org_hierarchy h
    WHERE h.channel_id = @ch_tt_emp
      AND h.effective_month = '2026-04-01'
      AND h.salesman_code = v.salesman_code
);
GO
...
-- ============================================================
-- B6.1 TRANSACTION: Incentive Detail — TT 5-level cascade sample
-- TT คงรูปแบบ single-sheet แต่ผลคำนวณแยก detail ครบ STAFF/SECT/DEPT/DIV/AD
-- ============================================================
DECLARE @run_tt INT = (
    SELECT cr.calc_run_id
    FROM dbo.trn_calc_run cr
    JOIN dbo.mst_period p ON p.period_id = cr.period_id
    JOIN dbo.mst_channel c ON c.channel_id = cr.channel_id
    WHERE p.period_code = 'FY2026-04' AND c.channel_code = 'TT'
);

IF @run_tt IS NOT NULL AND NOT EXISTS (SELECT 1 FROM dbo.trn_incentive_detail WHERE calc_run_id = @run_tt)
INSERT INTO dbo.trn_incentive_detail
    (calc_run_id, salesman_code, position_level_code, product_code,
     target_amount, actual_amount, achievement, shortage_flag, final_achievement,
     goal_multiplier, incentive_base, product_weight, incentive_amount)
VALUES
    -- STAFF level (TT001, TT002)
    (@run_tt,'TT001','STAFF','AJ', 120000,125000,1.0417,0,1.0417,1.0300,15000,0.3000,4635.00),
    (@run_tt,'TT001','STAFF','RD',  70000, 67000,0.9571,0,0.9571,0.9500, 8000,0.2000,1520.00),
    (@run_tt,'TT001','STAFF','BD',  85000, 88000,1.0353,0,1.0353,1.0300, 5000,0.1500, 772.50),
    (@run_tt,'TT002','STAFF','AJ', 130000,143000,1.1000,0,1.1000,1.1000,15000,0.3000,4950.00),
    (@run_tt,'TT002','STAFF','RD',  75000, 71000,0.9467,0,0.9467,0.9000, 8000,0.2000,1440.00),
    (@run_tt,'TT002','STAFF','BD',  90000, 92000,1.0222,0,1.0222,1.0000, 5000,0.1500, 750.00),

    -- SECT_MGR level (AVERAGEIFS จาก STAFF)
    (@run_tt,'TSM001','SECT_MGR','AJ', 250000,268000,1.0716,0,1.0716,1.0600,11000,0.3000,3498.00),
    (@run_tt,'TSM001','SECT_MGR','RD', 145000,138000,0.9517,0,0.9517,0.9500, 6000,0.2000,1140.00),
    (@run_tt,'TSM001','SECT_MGR','BD', 175000,180000,1.0286,0,1.0286,1.0000, 4200,0.1500, 630.00),

    -- DEPT_MGR level (AVERAGEIFS จาก SECT_MGR)
    (@run_tt,'TDM001','DEPT_MGR','AJ', 250000,268000,1.0716,0,1.0716,1.0600, 8500,0.3000,2703.00),
    (@run_tt,'TDM001','DEPT_MGR','RD', 145000,138000,0.9517,0,0.9517,0.9500, 4600,0.2000, 874.00),
    (@run_tt,'TDM001','DEPT_MGR','BD', 175000,180000,1.0286,0,1.0286,1.0000, 3400,0.1500, 510.00),

    -- DIV_MGR level (AVERAGEIFS จาก DEPT_MGR)
    (@run_tt,'TDV001','DIV_MGR','AJ', 250000,268000,1.0716,0,1.0716,1.0600, 7500,0.3000,2385.00),
    (@run_tt,'TDV001','DIV_MGR','RD', 145000,138000,0.9517,0,0.9517,0.9500, 4000,0.2000, 760.00),
    (@run_tt,'TDV001','DIV_MGR','BD', 175000,180000,1.0286,0,1.0286,1.0000, 3200,0.1500, 480.00),

    -- AD level (AVERAGEIFS จาก DIV_MGR)
    (@run_tt,'TAD001','AD','AJ', 250000,268000,1.0716,0,1.0716,1.0600, 6500,0.3000,2067.00),
    (@run_tt,'TAD001','AD','RD', 145000,138000,0.9517,0,0.9517,0.9500, 3500,0.2000, 665.00),
    (@run_tt,'TAD001','AD','BD', 175000,180000,1.0286,0,1.0286,1.0000, 2800,0.1500, 420.00);
GO
...
VALUES
    ...
    (@run_mt,'SP001',N'นาย ก. ใจดี','STAFF','MT','2026-06-01', 7261.00,0,0,0,0, 0.00,  7261.00,N'โอนเข้าบัญชี'),
    -- SP002: Staff incentive total = 4950+1648+825 = 7423
    (@run_mt,'SP002',N'นาย ข. รักชาติ','STAFF','MT','2026-06-01', 7423.00,0,0,0,0, 0.00,  7423.00,N'โอนเข้าบัญชี'),
    -- SM001 (Sect Mgr): cascade ระดับ Sect (ตัวอย่าง)
    (@run_mt,'SM001',N'นาย ค. มั่นคง','SECT_MGR','MT','2026-06-01',0, 3800.00,0,0,0, 0.00,  3800.00,N'โอนเข้าบัญชี');
GO

-- ============================================================
-- C1.1 OUTPUT: For HR Variable — TT Apr-2026 (5-level trace)
-- ============================================================
DECLARE @run_tt_var INT = (
    SELECT cr.calc_run_id
    FROM dbo.trn_calc_run cr
    JOIN dbo.mst_period p ON p.period_id = cr.period_id
    JOIN dbo.mst_channel c ON c.channel_id = cr.channel_id
...
IF @run_tt_var IS NOT NULL AND NOT EXISTS (SELECT 1 FROM dbo.out_for_hr_variable WHERE calc_run_id = @run_tt_var)
INSERT INTO dbo.out_for_hr_variable
    (calc_run_id, employee_code, employee_name_th, position_level_code, channel_code,
     variable_pay_month, incentive_staff, incentive_sect, incentive_dept, incentive_div, incentive_ad,
     gd_incentive_total, total_variable, payment_method)
VALUES
    (@run_tt_var,'TT001',N'นาง ง. สดใส','STAFF','TT','2026-06-01', 6927.50,0,0,0,0, 0.00, 6927.50,N'โอนเข้าบัญชี'),
    (@run_tt_var,'TT002',N'นาย จ. มีสุข','STAFF','TT','2026-06-01', 7140.00,0,0,0,0, 0.00, 7140.00,N'โอนเข้าบัญชี'),
    (@run_tt_var,'TSM001',N'นาย ฉ. คุมทีม','SECT_MGR','TT','2026-06-01', 0,5268.00,0,0,0, 0.00, 5268.00,N'โอนเข้าบัญชี'),
    (@run_tt_var,'TDM001',N'นาง ช. ดูแผนก','DEPT_MGR','TT','2026-06-01', 0,0,4087.00,0,0, 0.00, 4087.00,N'โอนเข้าบัญชี'),
    (@run_tt_var,'TDV001',N'นาย ฌ. รวมพลัง','DIV_MGR','TT','2026-06-01', 0,0,0,3625.00,0, 0.00, 3625.00,N'โอนเข้าบัญชี'),
    (@run_tt_var,'TAD001',N'นาง ญ. บริหาร','AD','TT','2026-06-01', 0,0,0,0,3152.00, 0.00, 3152.00,N'โอนเข้าบัญชี');
GO
...
UPDATE dbo.out_for_hr_variable
SET employee_name_th = CASE employee_code
        WHEN 'TT001' THEN N'นาง ง. สดใส'
        WHEN 'TT002' THEN N'นาย จ. มีสุข'
        WHEN 'TSM001' THEN N'นาย ฉ. คุมทีม'
        WHEN 'TDM001' THEN N'นาง ช. ดูแผนก'
        WHEN 'TDV001' THEN N'นาย ฌ. รวมพลัง'
        WHEN 'TAD001' THEN N'นาง ญ. บริหาร'
        ELSE employee_name_th
    END,
    payment_method = N'โอนเข้าบัญชี'
WHERE employee_code IN ('TT001','TT002','TSM001','TDM001','TDV001','TAD001');
