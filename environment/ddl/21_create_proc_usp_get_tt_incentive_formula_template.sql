...
/*
Purpose:
- Stored Procedure สำหรับทีม Business ใช้ตรวจสูตรคำนวณ TT ตาม ws_type และเดือนที่ต้องการ
- ครอบคลุม matrix, rate, goal threshold, option1 payout, special KPI

Result sets:
1) Section A: สูตรหลักราย Product
2) Section B: Goal Threshold
3) Section C: Option1 Band + Payout
4) Section D: Simulation payout
*/
CREATE OR ALTER PROCEDURE dbo.usp_get_tt_incentive_formula_template
    @WsType NVARCHAR(50) = N'TOP_WS',
    @PeriodCode NVARCHAR(20) = N'FY2026-05',
    @SalesMonth DATE = NULL,
    @AchievementForSimulation DECIMAL(9,4) = 1.0500
AS
BEGIN
    SET NOCOUNT ON;
