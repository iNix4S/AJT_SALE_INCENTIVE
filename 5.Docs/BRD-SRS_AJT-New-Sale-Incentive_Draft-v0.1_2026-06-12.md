...
เพื่อควบคุมขอบเขตโครงการในเฟสนี้และป้องกัน Scope Creep นิยาม Out-of-Scope ถูกกำหนดตามหลักคิดต่อไปนี้:

1. โครงการนี้โฟกัสการย้ายและควบคุมกระบวนการคำนวณ Incentive จาก Excel ไปสู่ระบบ (Calculation, Validation, Approval, Export)
2. โครงการนี้ไม่ครอบคลุมการเปลี่ยนแปลงระบบต้นทาง (BI/HCM) และไม่ครอบคลุมระบบปลายทางจ่ายเงินจริง
3. โครงการนี้ยึด policy ที่ยืนยันแล้วเป็น baseline สำหรับการพัฒนา ไม่ใช่การออกแบบ policy ใหม่ทั้งชุด

รายการ Out-of-Scope อย่างเป็นทางการ:

| รายการ | เหตุผลเชิงขอบเขต | ผลกระทบหากรวมในเฟสนี้ |
| --- | --- | --- |
| การจ่ายเงินจริงผ่านระบบ Payroll/Banking โดยตรง | ระบบนี้กำหนดบทบาทเป็น Incentive Calculation and Output System โดยส่งออกผลให้ HR/Payroll ดำเนินการจ่ายต่อ | เพิ่มความซับซ้อนด้านการเงิน/กฎหมาย/ควบคุมภายในเกินขอบเขตโครงการ |
| การปรับโครงสร้าง Job Function นอกเหนือข้อมูลที่รับมาจาก HCM | Job Function ถือเป็นข้อมูลตั้งต้นจาก HCM ในโครงการนี้ และใช้เพื่อคำนวณตาม policy ที่มีอยู่ | กระทบ Master Data Governance และต้องเปลี่ยนกระบวนการองค์กรข้ามทีม |
| การ redesign กระบวนการ BI/HCM ต้นทาง | BI/DWC และ HCM เป็น upstream systems ที่ต้องเชื่อมต่อผ่าน interface ที่ตกลง ไม่ใช่เป้าหมายการปรับโครงสร้างระบบต้นทาง | เพิ่ม dependency และ timeline สูง ทำให้เสี่ยงเลื่อนแผนส่งมอบ |
| การออกแบบ Incentive Policy ใหม่ทั้งหมด | เอกสารฉบับนี้ตั้งเป้าแปลง policy ที่ยืนยันแล้วให้เป็นระบบที่ตรวจสอบได้ ไม่ใช่เริ่ม policy design ใหม่ | เกิด requirement churn และทำให้ baseline สำหรับ UAT/Sign-off ไม่เสถียร |

หมายเหตุแหล่งที่มา:

- ข้อกำหนด Out-of-Scope ใน Section นี้เป็นการสังเคราะห์จาก Objective, In-Scope, Integration Requirements, และ Risks/Dependencies ของเอกสาร BRD/SRS ฉบับเดียวกัน
- ไม่ใช่ข้อความที่คัดลอกตรงจาก `1.General Documents` หรือ `4.System Analyst and Design`
